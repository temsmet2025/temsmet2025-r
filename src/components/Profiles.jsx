...
const Profiles = (props) => {
  ...
  return (
    <div className="w-full">
      ...
      <HeadingBar />
      <div id={`profiles-${props.title}`} className='flex flex-wrap justify-center sm:justify-between' >
        {data?.map((profile, index) => (
          <div key={index}>
            <div id={`card-${profile.name}-${index}`} className="relative w-64 lg:w-[22rem] h-96 lg:h-44 rounded-md border hover:rounded-xl hover:shadow-xl hover:bg-slate-300/80 hover:scale-105 transition-all duration-300 ease-in-out m-4 sm:m-5 group">
              ...
              <div className="card-body w-64 lg:w-[25rem] h-52 lg:h-44 absolute flex flex-col lg:flex-row items-center text-wrap ">
                ...
                <div className=" text-amber-950">
                  <p className='flex flex-col text-wrap text-center md:w-48'>
                    ...
                    {profile.affiliation != "" && <span className='text-base md:text-sm p-4' style={{whiteSpace:"pre-line"}}>{profile.affiliation}</span>}
                  </p> 
                </div>
              </div>
            </div>
